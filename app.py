...
def send_email(user, verification_code):
    ...
    print("enviando correo de verificación a: ", email)
...
def verify_code(message):
    ...
    if user_id in verification_codes:
        ...
        if (
            time.time() - timestamp <= 300
        ):  # Verifica si el código es válido dentro de los 5 minutos
            if input_code == stored_code:
                urls = find_files(username)
                bot.reply_to(
                    message,
                    f"¡Código de verificación correcto! Aquí está tu archivo(s). {urls}",
                )
            ...
            bot.reply_to(
                message,
                "El código de verificación ha expirado. Por favor, solicita uno nuevo con /download.",
            )
            del verification_codes[user_id]  # Elimina el código expirado
    else:
        bot.reply_to(
            message,
            "No hay un código de verificación asociado a tu cuenta. Por favor, solicita uno nuevo con /download.",
        )
